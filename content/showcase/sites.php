<section class="bg-white dark:bg-violet-200">
    <div class="flex flex-col py-8 px-4 mx-auto max-w-screen-xl lg:pb-4 lg:pt-10 lg:px-6">
        <div class="text-left lg:mb-10 mb-8">
            <h2 class="mb-1 text-3xl text-center lg:text-3xl tracking-tight font-semibold text-gray-900 dark:text-gray-900">Cari Website Referensimu Lads!</h2>
        </div> 
        <div class="grid gap-8 md:grid-cols-2 sm:grid-cols-1 lg:grid-cols-3 place-items-center">

        <!-- Lakukan Perulangan Data di sini -->
            <?php 
                $contents = execThis("SELECT cate, web_name, web_pict, category_id, web_url FROM websites INNER JOIN categories ON categories.id = websites.category_id");
                ...
                <div class="flex justify-end px-4 pt-4">
                    ...
                    <div id="dropdown" class="z-10 hidden text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-violet-200">
                        <ul class="py-2" aria-labelledby="dropdownButton">
                          <li>
                              <a href="<?= $content["web_url"] ?>" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-violet-300 dark:text-gray-900 dark:hover:text-violet-600">Kunjungi Website</a>
                          </li>
                        </ul>
                    </div>
                </div>
                <div class="flex flex-col items-center pb-10">
                    <img class="w-24 h-24 mb-3 rounded-full shadow-lg shadow-violet-300" src="./assets/img/web/<?= $content["web_pict"]; ?>" alt="<?= $content["web_pict"]; ?>"/>
                    <h5 class="mb-1 text-xl font-medium text-gray-900 dark:text-gray-900"><?= $content["web_name"]; ?></h5>
                    <span class="text-sm text-gray-500 dark:text-gray-400"><?= $content["cate"]; ?></span>
                    <div class="flex mt-4 space-x-3 md:mt-6">
                        <a href="<?= $content["web_url"] ?>" class="inline-flex items-center px-4 py-2 text-sm font-medium text-center text-white bg-violet-700 rounded-lg hover:bg-violet-800 focus:ring-4 focus:outline-none focus:ring-violet-300 dark:bg-violet-600 dark:hover:bg-violet-700 dark:focus:ring-violet-800">Kunjungi Website</a>
